...
object HolographicExportUtils {
    ...
    /**
     * Douglas-Peucker algorithm to simplify trace points. Reduces file size while preserving visual
     * fidelity of the path.
     */
     ...
     *
     * No template file is used — the entire HTML is built in Kotlin to avoid IDE auto-formatting
     * corruption of placeholders.
     */
    fun generateHolographicScrollContent(
        ...
        return try {
            ...
            val simplified =
                    if (tracePoints.size > 500) simplify(tracePoints, 0.00005) else tracePoints
            ...
            val shadow =
                    if (uiState.artTextBorder) "0 0 10px rgba(0,0,0,0.8), 0 0 5px rgba(0,0,0,1)"
            ...
            val html = buildString {
                ...
                append(".ov.pol{justify-content:flex-end;align-items:center;padding:0}")
                ...
                val frameBg =
                        when (uiState.polaroidFrameStyle) {
                            ...
                            "LIQUID_GLASS" -> "rgba(255,255,255,0.3)"
                            else -> "#FAFAFA"
                        }
                val glassEffect =
                ...
                if (uiState.polaroidInnerBorder > 0) {
                    ...
                    append(".pol-map-area{border:${uiState.polaroidInnerBorder}px solid $bc}")
                }

                // Watermark Styles
                val pColor = if (uiState.polaroidFrameStyle == "CLASSIC_BLACK") "#fff" else "#000"
                append(
                ...
                append(".pol-ttl{font-size:24px;font-weight:bold;margin-bottom:8px;opacity:0.9}")
                append(
                ...
                append(
                        ".ttl{font-size:22px;margin-bottom:3px;letter-spacing:1.5px;color:$textColorHex;font-family:$fontCss;font-style:$fontStyle;text-shadow:$shadow}"
                )
                append(
                ...
                when (layoutName) {
                    "POLAROID" -> {
                        ...
                        append(
                                "<div class=\"pol-meta\"><div>$safeMeta</div><div style=\"font-family:monospace;font-size:8px;margin-top:2px\">TOTAL DISTANCE: %.2f KM</div></div>".format(
                                        totalDistanceKm
                                )
                        )
                        append(
